...
export function SectionHeader({ sa, en, sub }) {
  return (
    <div style={{ textAlign: 'center', marginBottom: 28 }}>
      <h2 style={{
        fontFamily: 'var(--font-sanskrit)', fontSize: 28, color: 'var(--ink)',
        margin: '0 0 4px', fontWeight: 400, letterSpacing: 2,
      }}>{sa}</h2>
      <div style={{
        fontFamily: 'var(--font-body)', fontSize: 15, color: 'var(--burnt-sienna)',
...
export function SeverityBadge({ severity, sa }) {
  ...
  return (
    <span style={{
      ...
      color: c.border, letterSpacing: 0.5,
    }}>
      <span style={{
...
export function Nav({ active, setActive }) {
  return (
    <nav style={{
      ...
      borderBottom: '2px solid var(--burnt-sienna)', marginBottom: 36,
      flexWrap: 'wrap',
    }}>
      {NAV_TABS.map(t => (
        <button
...
export function LoadingSkeleton() {
  return (
    <div style={{
      ...
      <div style={{
        fontFamily: 'var(--font-sanskrit)', fontSize: 24,
        color: 'var(--ochre)', marginBottom: 8,
      }}>गणना चल रही है...</div>
      <div style={{
        fontFamily: 'var(--font-body)', fontSize: 13,
        color: 'var(--burnt-sienna)', fontStyle: 'italic',
      }}>Calculating planetary positions...</div>
    </div>
  );
}
