...
export default function KundliPage() {
  ...
  const handleSavePdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.setFontSize(14);
      pdf.text('Medini Jyotish', 105, 10, { align: 'center' });
      pdf.setFontSize(8);
      pdf.setTextColor(200, 180, 140);
      pdf.text('medinijyotish.com', 105, 15, { align: 'center' });

      // Content — handle multi-page
      let heightLeft = imgHeight;
      let position = 22; // start after header
      ...
      while (heightLeft > 0) {
        pdf.addPage();
        position = -(pageHeight - position) + (imgHeight - heightLeft - (pageHeight - 22));
        pdf.addImage(imgData, 'JPEG', 10, -(imgHeight - heightLeft - 5), imgWidth, imgHeight);
      ...
      pdf.text('Generated by medinijyotish.com | Based on Brihat Samhita & Parashara Hora Shastra', 105, 290, { align: 'center' });

      const fileName = kundli.name
        ? `Kundli_${kundli.name.replace(/\s+/g, '_')}.pdf`
        : 'Kundli_MediniJyotish.pdf';
      pdf.save(fileName);
    } catch (e) {
      ...
      {kundli && (
        <div style={{ marginTop: 24, animation: 'fadeSlideIn 0.5s ease' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 16 }}>
            <button onClick={handleSavePdf} disabled={pdfLoading} style={{
              fontFamily: 'var(--font-devanagari)', fontSize: 14, padding: '10px 28px',
              background: pdfLoading ? 'rgba(92,64,51,0.4)' : 'var(--burnt-sienna)',
              color: 'var(--parchment-light)', border: 'none',
              cursor: pdfLoading ? 'wait' : 'pointer', letterSpacing: 1,
              transition: 'all 0.2s',
            }}>
              {pdfLoading ? '⏳ PDF बन रहा है...' : '📄 Save as PDF — कुण्डली सहेजें'}
            </button>
          </div>
